...
if (file_get_contents($path)) {
	...
	function student_final_result_ninth($data){                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        
	    ...
	    $result['student'] = $this->db->query("SELECT `s`.*, `c`.`name` as `cname`, `sec`.`name` as `sec_name`
			FROM `student` `s`
			JOIN `class` `c` ON `c`.`c_id` = `s`.`class_id`
			JOIN `section` `sec` ON `sec`.`id` = `s`.`section`
			AND `s`.`school_id` = ".$school_id."
			AND `s`.`s_id` = ".$data['s_id']."
			AND `s`.`section` = ".$data['section_id']."")->result_array();
...
//                 			}
	                ...
	                if($temp['sub_name'] == "FIT"){
	                	$total_grade = ceil($temp['annual_mark'] + $temp['practical']);
	                }
	                else{
	                	$total_grade = ceil($temp['annualsub_total']);
	                }
	                if($total_grade >= 91){
	                	$temp['grade'] = 'A1';
	                }
	                else if($total_grade >= 81 && $total_grade <= 90){
	                	$temp['grade'] = 'A2';
	                }
	                else if($total_grade >= 71 && $total_grade <= 80){
	                	$temp['grade'] = 'B1';
	                }
	                else if($total_grade >= 61 && $total_grade <= 70){
	                	$temp['grade'] = 'B2';
	                }
	                else if($total_grade >= 51 && $total_grade <= 60){
	                	$temp['grade'] = 'C1';
	                }
	                else if($total_grade >= 41 && $total_grade <= 50){
	                	$temp['grade'] = 'C2';
	                }
	                else if($total_grade >= 33 && $total_grade <= 40){
	                	$temp['grade'] = 'D';
	                }
	                else{
	                	$temp['grade'] = 'E';
	...
	$final_array['aggrigate'] = $aggrigate;
	if(isset($back['get_extra'])){
		if($back['get_extra']){
	...
	function student_deatil_high_class_final($data){
		...
		$result['student'] = $this->db->query("select s.s_id,s.admission_no,s.session,s.roll_no,s.school_id,s.class_id,s.section,s.name,s.father_name,s.mother_name,s.gender,s.cast,s.contact_no,s.email_id,s.aadhar,s.height,s.weight,s.address,s.medium,s.subject_group,s.fit,s.elective,s.house,s.hostler,s.blood_group,s.guardian,s.local_address,s.medical,s.tc,s.photo,s.admission_date,s.created_at,s.created_by,s.ip,DATE_FORMAT(s.dob,'%d-%m-%Y') as dob,c.name as cname,sec.name as sec_name
			FROM `student` `s`
			JOIN `class` `c` ON `c`.`c_id` = `s`.`class_id`
			JOIN `section` `sec` ON `sec`.`id` = `s`.`section`
			AND `s`.`school_id` = ".$data['school_id']."
...
// 			}
				...
				foreach($marks_final as $mark_final){
					foreach($final_marks_masters as $fmm){
						if($mark_final['subject_id'] == $fmm['subject']){
							...
							$temp['a_marks'] = $mark_final['a_marks'];
		...
		$extra_counter = 0;
		$final_array = array();
		$student_marks_pre = array_map("unserialize", array_unique(array_map("serialize", $student_marks_pre)));
		$student_marks_mid = array_map("unserialize", array_unique(array_map("serialize", $student_marks_mid)));
		$student_marks_post = array_map("unserialize", array_unique(array_map("serialize", $student_marks_post)));
		$student_marks_final = array_map("unserialize", array_unique(array_map("serialize", $student_marks_final)));
				...
				foreach($student_marks_post as $smpo){
					foreach($student_marks_final as $smf){
						if($smp['subject_id'] == $smm['subject_id'] && $smm['subject_id'] == $smpo['subject_id'] && $smm['subject_id'] == $smf['subject_id']){
							...
							$temp['academic_attention'] = $smf['a_marks'];
							...
							$outoff = $outoff + $temp['final_thory_marks_max'];
...
// 									}
		...
		$marks_final = array_map("unserialize", array_unique(array_map("serialize", $marks_final)));
		
		$result['final_marks'] = $marks_final;
		$result['co_marks'] = $final_co_marks;
		$result['total_obtail'] = $total_obtain_marks;
		$result['outoff'] = $outoff;
		$result['percent'] = round((($result['total_obtail'] * 100)/ 500),2);
...
// 										}
			...
			$result['final_marks'] = $marks_final;
			$result['co_marks'] = $final_co_marks;
			$result['total_obtail'] = round($total_obtain_marks,2);
			$result['outoff'] = $outoff;
...
// 					else{
	...
	//////////////////////////////////////////////////////////////////////////////
	function student_final_result_ninth_loop($data){
		if($data['class_id'] > 3 && $data['class_id'] < 9){
			$class_category = '1-5';
		}
		else if($data['class_id'] > 8 && $data['class_id'] < 13){
		...
		$students = $this->db->get_where('student',array('session'=>$data['session'],'medium'=>$data['medium'],'school_id'=>$data['school'],'class_id'=>$data['class_id'],'section'=>$data['section_id']))->result_array();
		...
		foreach($students as $student){ 
			...
			$this->db->join('subject s','s.sub_id= sm.subject_id');
			$this->db->where_in('sm.mm_id',$mm_ids);
			$student_marks = $this->db->get_where('student_mark sm',array('sm.student_id'=>$student['s_id']))->result_array();
