			...
			<div class="box-body table-responsive no-padding">
              <table class="table table-hover text-center t-input-center">
                <thead><tr>
                  ...
                  <th class="academic" style="display:none;">Academic Attention</th>
                </tr>
				</thead>
...
$(document).on('click','#fetch_student',function(){
	...
	if(formvalid){
		$.ajax({
			type: 'POST',
			url: baseUrl+'Student_ctrl/student_list_marks_high_class',
			dataType: "json",
			data: {
				'class' : $('#class').val(),
				'section' : $('#section').val(),
			...
			success: function(response) {
				...
				if(response.status == 200){
					if(response.s_marks > 0){
						$('#max_mark').html('Maxium Subject Mark : ' + response.s_marks).css('display','block');	
					}
					else{
					...
					if(response.a_marks > 0){
						$('#max_academic').html('Maxium Academic Attention  : ' + response.a_marks).css('display','block');
					}
					else{
						$('#max_academic').css('display','none');
					}
					...
					$.each(response.data,function(key,value){
						x = x + '<tr>'+
									...
									if (typeof value.marks === 'undefined') {
										x = x + '<td><input type="text" data-s_id="'+ value.s_id +'" class="s_marks" data-max="'+ response.s_marks +'" value="" max ="'+ response.s_marks +'" size="4"></td>';
									}
									else{
										x = x + '<td><input type="text" data-s_id="'+ value.s_id +'" class="s_marks" value="'+ value.marks +'" data-max="'+ response.s_marks +'" max ="'+ response.s_marks +'" size="4"></td>';
									}
	
									if (typeof value.p_marks === 'undefined') {
										x = x + '<td><input type="text" data-s_id="'+ value.s_id +'" class="p_marks practical" data-max="'+ response.p_marks +'" max ="'+ response.p_marks +'" size="4"></td>';
									}
									else{
										x = x + '<td><input type="text" data-s_id="'+ value.s_id +'" class="p_marks practical" value="'+ value.p_marks +'" data-max="'+ response.p_marks +'" max ="'+ response.p_marks +'" size="4"></td>';
									}
									if(typeof value.a_marks === 'undefined'){
										x = x + '<td><input type="text" data-s_id="'+ value.s_id +'" class="a_marks academic" data-max="'+ response.a_marks +'" max ="'+ response.a_marks +'" size="4"></td>';
									}
									else{
										x = x + '<td><input type="text" data-s_id="'+ value.s_id +'" class="a_marks academic" value="'+ value.a_marks +'" data-max="'+ response.a_marks +'" max ="'+ response.a_marks +'" size="4"></td>';
									}
								'</tr>'; 
						c = c + 1;
					});
...
$(document).on('click','#subject_mark',function(){
	...
	if($('.practical').css('display') == 'block'){
		$(".p_marks").each(function() {
			var temp = {};
			temp.val = $(this).val();
			temp.s_id = $(this).data('s_id');
		    p_mark.push(temp);
	...
	if($('.academic').css('display') == 'block'){
		$(".a_marks").each(function() {
			var temp = {};
			temp.val = $(this).val();
			temp.s_id = $(this).data('s_id');
		    a_mark.push(temp);
	...
	mark_entry(p_mark,stud_mark,a_mark);
});

function mark_entry(p_mark,stud_mark,a_mark){
	var s_group = $('#s_group').val();
	var elective = $('#elective').val();
	var subject = $('#subject').val();
	...
	$.ajax({
		type: 'POST',
		url: baseUrl+'Student_ctrl/marks_entry_high_class',
		dataType: "json",
		data: {
			'class' : $('#class').val(),
			'section' : $('#section').val(),
...
$(document).on('keyup','.s_marks,.practical,.a_marks',function(){
...
$(document).on('blur','.s_marks,.practical,.a_marks',function(){
	var max = $(this).data('max');
	var val = $(this).val();
	if(val > max || val == ''){
		$(this).css('box-shadow','0px 0px 10px red');
...
$(document).on('keypress','.s_marks,.p_marks,.a_marks',function(e){
...
$(document).on('keyup','.s_marks,.p_marks,.a_marks',function(){
	var max = $(this).data('max');
  	var val = $(this).val();
...
$(document).on('change','#elective',function(){
	if($(this).val() == 23){
		//$("#subject option[value='10']").remove();
		//$("#subject option[value='11']").remove();
		//$("#subject option[value='12']").remove();
		$('#subject').val(9);
		
	}
	if($(this).val() == 26){
		//$("#subject option[value='9']").remove();
		//$("#subject option[value='10']").remove();
		//$("#subject option[value='12']").remove();
		$('#subject').val(11);
	}
