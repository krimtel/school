...
$(document).on('click','#fetch_student',function(){
	...
	if(formvalid){
		$.ajax({
			type: 'POST',
			url: baseUrl+'Student_ctrl/compart_student_list_marks',
			dataType: "json",
			data: {
				'class' : class_id,
				'medium' : medium,
							...
							x = x + '<tr>'+
										...
										if(response.flag){
											...
											if (typeof value.marks != 'undefined'){	
												...
												x = x + '<td><input type="text" class="subject_mark_box" min="0" data-s_id="'+ value.s_id +'" data-max="'+ response.max +'" max="'+ response.max +'" value="'+ value.n_marks +'" required></td>';
											}
										}
										if(response.s_type == 'Scholastic'){
											if($('#e_type').val() == 4 || $('#e_type').val() == 9){
												if(f){
													...
													$('#max_notebook').html('Maximum Notebook Mark :'+ response.internal_marks + ' ').hide();
													$('#max_enrich').html('Maximum Enrichment Mark :' + response.internal_marks + ' ').hide();
												}
...
function atten(stud_mark){
	form_value();
	  $.ajax({
			type: 'POST',
			url: baseUrl+'Subject_ctrl/compart_marks_entry',
			dataType: "json",
			data: {
				'medium': medium,
				'class' : class_id,
