...
class Marks_entry_cc extends CI_Controller {
    ...
    public function index(){
        ...
        $data['subject_list']=$this->std_model->subject_list();
    ...
    public function search_list(){
		...
		if(($e_type_name == 'FA1' || $e_type_name =='FA2' || $e_type_name =='FA3' || $e_type_name =='FA4') && ($sub_type=='1')){
				$max="20";
		}else if(($e_type_name=='FA1' || $e_type_name=='FA2' || $e_type_name=='FA3' || $e_type_name=='FA4')&& ($sub_type=='2')){
			$max="3";
			}
		else if(($e_type_name=='SA1' || $e_type_name=='SA2') && ($sub_type=='1')){
			$max="80";
			}
		elseif(($e_type_name=='SA1' || $e_type_name=='SA2') && ($sub_type=='2')){
				$max="3";
			}
			
    	 $this->load->model('student_model_cc','std_model');
    	$result = $this->std_model->search_list($school_id, $medium, $e_type, $class, $section, $subject);
    	if(count($result)>0){
